/*
 * linux/arch/arm/mach-omap2/board-dham35xebox.c
 *
...
#define dham35xebox_MDIO_FREQUENCY	(1000000)

static struct mdio_platform_data dham35xebox_mdio_pdata = {
	.bus_freq	= dham35xebox_MDIO_FREQUENCY,
};
...
static struct platform_device am3517_mdio_device = {
	...
	.dev.platform_data = &dham35xebox_mdio_pdata,
};

static struct emac_platform_data dham35xebox_emac_pdata = {
	.rmii_en	= 1,
};
...
static void dham35xebox_ethernet_init(struct emac_platform_data *pdata)
{
	unsigned int regval;
	int r;
...
static struct i2c_board_info __initdata dham35xebox_i2c1_boardinfo[] = {
	{
...
static struct i2c_board_info __initdata dham35xebox_i2c2_boardinfo[] = {
	{
...
static int __init dham35xebox_i2c_init(void)
{
	...
	if (gpio_request_one(TSC2004_IRQGPIO, GPIOF_IN, "TS IRQ") < 0) {
		...
		omap_register_i2c_bus(1, 400, dham35xebox_i2c1_boardinfo,
				ARRAY_SIZE(dham35xebox_i2c1_boardinfo));
	}


	omap_register_i2c_bus(2, 100, dham35xebox_i2c2_boardinfo,
				ARRAY_SIZE(dham35xebox_i2c2_boardinfo));
...
static struct gpio dham35xebox_dss_gpios[] __initdata = {

	/* GPIO 141 = LCD Panel Power enable pin */
	{ LCD_PANEL_EN,	 GPIOF_OUT_INIT_LOW, "lcd en" },
};

static int dham35xebox_panel_enable_lcd(struct omap_dss_device *dssdev)
{
	gpio_set_value(LCD_PANEL_EN, 1);
	return 0;
}

static void dham35xebox_panel_disable_lcd(struct omap_dss_device *dssdev)
{
...
static struct panel_generic_dpi_data lcd_panel = {
	...
	.platform_enable	= dham35xebox_panel_enable_lcd,
	.platform_disable	= dham35xebox_panel_disable_lcd,
};

static struct omap_dss_device dham35xebox_lcd_device = {
	.name			= "lcd",
...
static struct omap_dss_device *dham35xebox_dss_devices[] = {
	&dham35xebox_lcd_device,
};

static struct omap_dss_board_info dham35xebox_dss_data = {
	.num_devices	= ARRAY_SIZE(dham35xebox_dss_devices),
	.devices	= dham35xebox_dss_devices,
	.default_device	= &dham35xebox_lcd_device,
};

static void __init dham35xebox_display_init(void)
{
	int r;

	omap_mux_init_gpio(LCD_PANEL_EN, OMAP_PIN_OUTPUT);

	r = gpio_request_array(dham35xebox_dss_gpios,
			       ARRAY_SIZE(dham35xebox_dss_gpios));
	if (r) {
	...
	omap_display_init(&dham35xebox_dss_data);
}

static int otg_host=1;

static int __init dham35xebox_otg_mode(char *options)
{
...
__setup("otg_mode=", dham35xebox_otg_mode);
...
static struct gpio dham35xebox_usb_gpios[] __initdata = {

	/* GPIO 117 = GPIO B Power pin , also on leds */
	{ USB_PWR_GPIO,	 GPIOF_OUT_INIT_HIGH, "usb pwr" },
};

static __init void dham35xebox_musb_init(void)
{
	int r;
	u32 devconf2;
	...
	r = gpio_request_array(dham35xebox_usb_gpios,
			       ARRAY_SIZE(dham35xebox_usb_gpios));
	if (r) {
...
static struct ti_hecc_platform_data dham35xebox_hecc_pdata = {
...
static void dham35xebox_hecc_init(struct ti_hecc_platform_data *pdata)
{
...
static struct mtd_partition dham35xebox_nand_partitions[] = {
	{
		.name           = "xloader",
		.offset         = 0,
...
static void __init dham35xebox_init_nand(void) {
	omap_nand_flash_init(0, dham35xebox_nand_partitions,
		 ARRAY_SIZE(dham35xebox_nand_partitions));
}
...
static inline void dham35xebox_init_hsmmc(void) {

	omap2_hsmmc_init(mmc);
}

/* #define USE_SPIFLASH_PARTITIONS */
static struct mtd_partition dham35xebox_spiflash_part[] = {
...
static struct flash_platform_data dham35xebox_spiflash_data = {
	.name		= "m25p80",
	.parts		= dham35xebox_spiflash_part,
	.nr_parts	= ARRAY_SIZE(dham35xebox_spiflash_part),
	.type		= "sst25vf032b",
};

static struct omap2_mcspi_device_config dham35xebox_spiflash_cfg = {
	.turbo_mode = 0,
	.single_channel = 1,
};

static struct spi_board_info dham35xebox_spi_info[] = {
/*	{
		.modalias		= "m25p80",
		.platform_data		= &dham35xebox_spiflash_data,
		.controller_data	= &dham35xebox_spiflash_cfg,
		.mode			= SPI_MODE_0,
...
static struct omap_board_config_kernel dham35xebox_config[] __initdata = {
};

static int backlight_invert=0;

static int __init dham35xebox_bl_invert(char *options)
{
...
__setup("pwm_pol=", dham35xebox_bl_invert);

static int framebuffer_depth=0;

static int __init dham35xebox_fb_depth(char *options)
{
...
__setup("fb_depth=", dham35xebox_fb_depth);

int dham35xebox_get_framebuffer_depth(void)
{
	return framebuffer_depth;
}
EXPORT_SYMBOL(dham35xebox_get_framebuffer_depth);

static void __init dham35xebox_init(void)
{
	omap_board_config = dham35xebox_config;
	omap_board_config_size = ARRAY_SIZE(dham35xebox_config);
	omap3_mux_init(board_mux, OMAP_PACKAGE_ZCN);

	omap_serial_init();

	dham35xebox_display_init();

	dham35xebox_i2c_init();

	dham35xebox_init_nand();

	dham35xebox_musb_init();

	usbhs_init(&usbhs_bdata);

	dham35xebox_init_hsmmc();

	dham35xebox_hecc_init(&dham35xebox_hecc_pdata);

	spi_register_board_info(dham35xebox_spi_info,
			ARRAY_SIZE(dham35xebox_spi_info));

	dham35xebox_ethernet_init(&dham35xebox_emac_pdata);
...
static void __init dham35xebox_reserve(void)
{
	omap_reserve();
}

MACHINE_START(DHAM35XEBOX, "DHAM35XEBOX")
	.atag_offset	= 0x100,
	.reserve	= dham35xebox_reserve,
	.map_io		= omap3_map_io,
	.init_early	= omap3_init_early,
	.init_irq	= omap3_init_irq,
	.init_machine	= dham35xebox_init,
	.timer		= &omap3_timer,
MACHINE_END
