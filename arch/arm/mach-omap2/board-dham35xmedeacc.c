/*
 * linux/arch/arm/mach-omap2/board-dham35xmedeacc.c
 *
...
#define dham35xmedeacc_MDIO_FREQUENCY	(1000000)

static struct mdio_platform_data dham35xmedeacc_mdio_pdata = {
	.bus_freq	= dham35xmedeacc_MDIO_FREQUENCY,
};
...
static struct platform_device am3517_mdio_device = {
	...
	.dev.platform_data = &dham35xmedeacc_mdio_pdata,
};

static struct emac_platform_data dham35xmedeacc_emac_pdata = {
	.rmii_en	= 1,
};
...
static void dham35xmedeacc_ethernet_init(struct emac_platform_data *pdata)
{
	unsigned int regval;
	int r;
...
static struct at24_platform_data eeprom_info2 = {
	.byte_len	= (4096) / 8,
	.page_size	= 16,
	.flags		= 0,
};

static struct i2c_board_info __initdata dham35xmedeacc_i2c1_boardinfo[] = {
	{
...
static struct i2c_board_info __initdata dham35xmedeacc_i2c2_boardinfo[] = {
	{
...
static int __init dham35xmedeacc_i2c_init(void)
{
	...
	if (gpio_request_one(TSC2004_IRQGPIO, GPIOF_IN, "TS IRQ") < 0) {
		...
		omap_register_i2c_bus(1, 400, dham35xmedeacc_i2c1_boardinfo,
				ARRAY_SIZE(dham35xmedeacc_i2c1_boardinfo));
	}


	omap_register_i2c_bus(2, 100, dham35xmedeacc_i2c2_boardinfo,
				ARRAY_SIZE(dham35xmedeacc_i2c2_boardinfo));
...
static struct gpio dham35xmedeacc_dss_gpios[] __initdata = {

	/* GPIO 141 = LCD Panel Power enable pin */
	{ LCD_PANEL_EN,	 GPIOF_OUT_INIT_LOW, "lcd en" },
};

static int dham35xmedeacc_panel_enable_lcd(struct omap_dss_device *dssdev)
{
	gpio_set_value(LCD_PANEL_EN, 1);
	return 0;
}

static void dham35xmedeacc_panel_disable_lcd(struct omap_dss_device *dssdev)
{
...
static struct panel_generic_dpi_data lcd_panel = {
	...
	.platform_enable	= dham35xmedeacc_panel_enable_lcd,
	.platform_disable	= dham35xmedeacc_panel_disable_lcd,
};

static struct omap_dss_device dham35xmedeacc_lcd_device = {
	.name			= "lcd",
...
static struct omap_dss_device *dham35xmedeacc_dss_devices[] = {
	&dham35xmedeacc_lcd_device,
};

static struct omap_dss_board_info dham35xmedeacc_dss_data = {
	.num_devices	= ARRAY_SIZE(dham35xmedeacc_dss_devices),
	.devices	= dham35xmedeacc_dss_devices,
	.default_device	= &dham35xmedeacc_lcd_device,
};

static void __init dham35xmedeacc_display_init(void)
{
	int r;

	omap_mux_init_gpio(LCD_PANEL_EN, OMAP_PIN_OUTPUT);

	r = gpio_request_array(dham35xmedeacc_dss_gpios,
			       ARRAY_SIZE(dham35xmedeacc_dss_gpios));
	if (r) {
	...
	omap_display_init(&dham35xmedeacc_dss_data);
}

static int otg_host=1;

static int __init dham35xmedeacc_otg_mode(char *options)
{
...
__setup("otg_mode=", dham35xmedeacc_otg_mode);
...
static struct gpio dham35xmedeacc_usb_gpios[] __initdata = {

	/* GPIO 117 = GPIO B Power pin , also on leds */
	{ USB_PWR_GPIO,	 GPIOF_OUT_INIT_HIGH, "usb pwr" },
};

static __init void dham35xmedeacc_musb_init(void)
{
	int r;
	u32 devconf2;
	...
	r = gpio_request_array(dham35xmedeacc_usb_gpios,
			       ARRAY_SIZE(dham35xmedeacc_usb_gpios));
	if (r) {
...
static struct ti_hecc_platform_data dham35xmedeacc_hecc_pdata = {
...
static void dham35xmedeacc_hecc_init(struct ti_hecc_platform_data *pdata)
{
...
static struct mtd_partition dham35xmedeacc_nand_partitions[] = {
	{
		.name           = "xloader",
		.offset         = 0,
...
static void __init dham35xmedeacc_init_nand(void) {
	omap_nand_flash_init(0, dham35xmedeacc_nand_partitions,
		 ARRAY_SIZE(dham35xmedeacc_nand_partitions));
}
...
static inline void dham35xmedeacc_init_hsmmc(void) {

	omap2_hsmmc_init(mmc);
}

/* #define USE_SPIFLASH_PARTITIONS */
static struct mtd_partition dham35xmedeacc_spiflash_part[] = {
...
static struct flash_platform_data dham35xmedeacc_spiflash_data = {
	.name		= "m25p80",
	.parts		= dham35xmedeacc_spiflash_part,
	.nr_parts	= ARRAY_SIZE(dham35xmedeacc_spiflash_part),
	.type		= "sst25vf032b",
};

static struct omap2_mcspi_device_config dham35xmedeacc_spiflash_cfg = {
	.turbo_mode = 1,
	.single_channel = 1,
};

static struct spi_board_info dham35xmedeacc_spi_info[] = {
/*	{
		.modalias		= "m25p80",
		.platform_data		= &dham35xmedeacc_spiflash_data,
		.controller_data	= &dham35xmedeacc_spiflash_cfg,
		.mode			= SPI_MODE_0,
...
static struct omap_board_config_kernel dham35xmedeacc_config[] __initdata = {
};

static int backlight_invert=0;

static int __init dham35xmedeacc_bl_invert(char *options)
{
...
__setup("pwm_pol=", dham35xmedeacc_bl_invert);

static int framebuffer_depth=0;

static int __init dham35xmedeacc_fb_depth(char *options)
{
...
__setup("fb_depth=", dham35xmedeacc_fb_depth);

int dham35xmedeacc_get_framebuffer_depth(void)
{
	return framebuffer_depth;
}
EXPORT_SYMBOL(dham35xmedeacc_get_framebuffer_depth);

static void __init dham35xmedeacc_init(void)
{
	omap_board_config = dham35xmedeacc_config;
	omap_board_config_size = ARRAY_SIZE(dham35xmedeacc_config);
	omap3_mux_init(board_mux, OMAP_PACKAGE_ZCN);

	omap_serial_init();

	dham35xmedeacc_display_init();

	dham35xmedeacc_i2c_init();

	dham35xmedeacc_init_nand();

	dham35xmedeacc_musb_init();

	usbhs_init(&usbhs_bdata);

	dham35xmedeacc_init_hsmmc();

	dham35xmedeacc_hecc_init(&dham35xmedeacc_hecc_pdata);

	spi_register_board_info(dham35xmedeacc_spi_info,
			ARRAY_SIZE(dham35xmedeacc_spi_info));

	dham35xmedeacc_ethernet_init(&dham35xmedeacc_emac_pdata);
...
static void __init dham35xmedeacc_reserve(void)
{
	omap_reserve();
}

MACHINE_START(DHAM35XMEDEACC, "DHAM35XMEDEACC")
	.atag_offset	= 0x100,
	.reserve	= dham35xmedeacc_reserve,
	.map_io		= omap3_map_io,
	.init_early	= omap3_init_early,
	.init_irq	= omap3_init_irq,
	.init_machine	= dham35xmedeacc_init,
	.timer		= &omap3_timer,
MACHINE_END
